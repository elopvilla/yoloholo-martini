...
namespace YoloHolo.Services
{
    ...
    public class MLImageAcquiringService : BaseServiceWithConstructor, IImageAcquiringService
    {
        ...
        {
            GameObject.Find("TextDebug").GetComponent<TextMeshProUGUI>().text += "<br>(ML)Starting camera capture";
            StartCameraCapture();
        }
            ...
            MLCameraBase.OutputFormat outputFormat = MLCameraBase.OutputFormat.RGBA_8888)
        {
            ...
            {
                GameObject.Find("TextDebug").GetComponent<TextMeshProUGUI>().text += "<br>(ML)Camera is available";
                await ConnectAndConfigureCameraAsync(cameraIdentifier, targetFrameRate, outputFormat);
            } else {
                GameObject.Find("TextDebug").GetComponent<TextMeshProUGUI>().text += "<br>(ML)Camera is NOT available";
            }
        }
